...
- Поля «Состояние/Решения» бери ТОЛЬКО из фактического `git`/`DEVLOG`/`BACKLOG`. Если git-команда падает с ошибкой (напр. `improper chunk offset`) — пиши `состояние не подтверждено: <текст ошибки>`, НЕ выдумывай из памяти.
