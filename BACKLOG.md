...
- [ ] Goal Entity Шаг 3 — граф связей целей: таблица KPI → KPI с полем `link_type ∈ {contributes, constrains, depends_on}` (индивидуальный KPI-источник / командная цель-потребитель, направление не ограничено «только вверх»); детект циклов графа связей (не запрет — флаг + подсветка); судья цикла = ближайший общий предок узлов цикла по `parent_id`, ручной флаг «подтверждён как ставка»; на этом шаге проработать соотношение структурной свёртки ресурсов (`resource.kind`, ADR-0002) и смысловой увязки по графу связей (см. Management_Model §9.8/9.9–9.11, ADR-0004). **Предусловие:** diff-sync в `patch_goal` (см. пункт ниже) должен быть сделан до этого шага. Модель — `docs/adr/0004-goal-link-types-cycles-composite-kpi.md` (закрывает все 4 открытых вопроса ADR-0003).
