...
- [x] Goal Entity Шаг 1 — CRUD + автодетекция fog/defined (`compute_definiteness`), см. ветку `feat/goal-entity-step1-crud`.
...
- [ ] Привязка frontend-карты целей (`frontend/src/os/data.ts`, сейчас демо-данные) к реальному Goal API.
