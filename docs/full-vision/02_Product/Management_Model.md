...
**Добавлено:** 2026-07-08 · **ADR:** [`docs/adr/0003-kpi-entity-and-goal-alignment.md`](../../adr/0003-kpi-entity-and-goal-alignment.md) (там же — открытые вопросы этого раздела)

Раздел фиксирует модель KPI и того, как цели узлов связаны друг с другом смыслово — отдельно от структурной принадлежности дерева (раздел 1.1). Это канон для будущих Шагов 2a/2b/3 (см. `BACKLOG.md`); реализация не должна домысливать то, что здесь не решено (раздел 9.8).
...
flowchart TD
    ...
    Proc -. владеет .-> K4
    Sales -. владеет .-> K5
    K3 ==связь: работает на==> K1
    K5 ==связь: работает на==> K1
    K4 ==связь: работает на==> K2
```

Пунктирные рёбра «владеет» — структурная принадлежность KPI своему узлу (не путать со связью целей); жирные рёбра «связь: работает на» — граф связей поверх структуры. `[K4]→[K2]` намеренно пересекает ветви дерева.
