...
## Оглавление
- [0. Назначение](#0-назначение) — зачем документ, что канон, а что открытый вопрос (раздел 7); прототип во frontend/src/os/ — демо-заготовка, не эталон.
- [1. Два типа карт (Ф1)](#1-два-типа-карт-ф1) — дерево-декомпозиция (отношение «содержит») vs воркфлоу-навык (BPMN-граф шагов на листьях), и как проваливаться между ними.
- [2. Роли и поток управления (Ф2)](#2-роли-и-поток-управления-ф2) — собственник → менеджмент → исполнитель → юнит; ярлыки ответственности, не иерархия; типы юнита по ADR-0006/0007.
- [3. Увязка (reconciliation) — мягко-жёсткая (Ф3)](#3-увязка-reconciliation--мягко-жёсткая-ф3) — принцип «система предлагает, человек утверждает»; ресурсный слой (people/money/other); разрыв подсвечивается, не блокирует; туман войны / бэклог.
- [4. Увязка через опрос офисных агентов (Ф4)](#4-увязка-через-опрос-офисных-агентов-ф4) — поздний срез (M7–M8): агент-советник маршрутизирует вопросы фронт/мидл/бэк-офисам, получает аналитические справки.
- [5. Навык ≠ Компетенция (Ф5)](#5-навык--компетенция-ф5) — жёсткое разведение терминов: навык = воркфлоу-сущность, компетенция = оценка покрытия юнит↔задача; правило переименования в UI/коде.
- [6. Всё есть Сущность (Ф6)](#6-всё-есть-сущность-ф6) — таблица маппинга объектов модели управления на Entity Platform (Goal/Skill/Competency/Unit/Resource/AI Employee/Document/Decision).
- [7. Увязка: статус открытых вопросов (Ф7)](#7-увязка-статус-открытых-вопросов-ф7) — какие из 7 исходных вопросов решены в первом срезе (ADR-0002) и какие отложены (роли доступа, версионирование справок, каталог навыков).
- [8. Расхождения с PRD (для сведения; приоритет у PRD)](#8-расхождения-с-prd-для-сведения-приоритет-у-prd) — 5 зафиксированных расхождений с PRD (ролевые ярлыки, термин «навык», туман войны/бэклог, геймификация, типы юнита) и их трактовка.
- [9. KPI-сущность и связи целей (Ф8)](#9-kpi-сущность-и-связи-целей-ф8) — KPI как отдельная Сущность; индивидуальная vs командная цель (ссылка на вышестоящий KPI); связи целей, циклы, составной KPI (ADR-0003/0004).
